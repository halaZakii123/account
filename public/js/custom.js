$(document).ready(function(){
    ...
    $('#sub_details').on('keyup blur', '.debit', function () {
        let $row = $(this).closest('tr');
        let debit = $row.find('.debit').val() || 0;

        $row.find('.row_sub_total').val((debit).toFixed(2));
    ...
    $('#sub_details').on('keyup blur', '.unit_price', function () {
        let $row = $(this).closest('tr');
        let quantity = $row.find('.quantity').val() || 0;
        let unit_price = $row.find('.unit_price').val() || 0;
